...
/// <summary>
/// 定义实现 <see cref="IOptionsNotifier"/> 的 HMAC 哈希选项。
/// </summary>
public class HmacHashOptions : AbstractOptionsNotifier
{
    /// <summary>
    /// 构造一个独立属性通知器的 <see cref="HmacHashOptions"/>（此构造函数适用于独立使用 <see cref="HmacHashOptions"/> 的情况）。
    /// </summary>
    /// <param name="sourceAliase">给定的源别名（独立属性通知器必须命名实例）。</param>
    public HmacHashOptions(string sourceAliase)
        : base(sourceAliase)
    {
    }
