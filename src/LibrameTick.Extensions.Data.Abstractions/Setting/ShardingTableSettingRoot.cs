...
public class ShardingTableSettingRoot : ISettingRoot
{
    ...
    /// 分表设置集合。
    /// </summary>
    public List<ShardingTableSetting> Tables { get; set; } = [];
    ...
    public virtual (ShardingTableSetting tableSetting, ShardingItemSetting itemSetting) GetOrCreate(
        ShardingDescriptor descriptor, string shardedName, string? sourceId, object? source, Action? createdAction)
    {
        ...
        else
        {
            ...
            table = new ShardingTableSetting(descriptor);
            table.Items.Add(item);
    ...
    public virtual bool TryAdd(ShardingTableSetting table)
    {
    ...
    public virtual bool TryGet(Type entityType,
