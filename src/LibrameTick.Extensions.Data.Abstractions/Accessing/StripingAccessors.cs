...
/// <summary>
/// 定义一个表示数据访问集合的分布式存取器，默认通过 <see cref="TransactionTraversalDispatcher{IAccessor}"/> 实现（支持针对读取与写入的分布式事务遍历等功能）。
/// </summary>
public class StripingAccessors : RedundableAccessors
{
    ...
    public override IList<TEntity> FindsWithSpecification<TEntity>(ISpec<TEntity>? specification = null)
        where TEntity : class
    ...
    public override async Task<IList<TEntity>> FindsWithSpecificationAsync<TEntity>(ISpec<TEntity>? specification = null,
