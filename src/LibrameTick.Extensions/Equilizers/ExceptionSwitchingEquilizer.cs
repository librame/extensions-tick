...
/// <summary>
/// 继承自 <see cref="AbstractEquilizer{TSource}"/> 的异常切换均衡器（执行单个来源）。
/// </summary>
/// <remarks>
/// 说明：每次调用单个来源的指定成员处理动作或方法。当处理发生异常，可自行依次切换到下一个来源处理；当切换次数达到指定的重试次数时，将抛出异常。
/// </remarks>
/// <typeparam name="TSource">指定的来源类型。</typeparam>
public class ExceptionSwitchingEquilizer<TSource> : AbstractEquilizer<TSource>
{
    ...
    /// <summary>
    /// 构造一个 <see cref="ExceptionSwitchingEquilizer{TSource}"/> 实例。
    /// </summary>
    /// <param name="sources">给定的 <typeparamref name="TSource"/> 集合。</param>
    public ExceptionSwitchingEquilizer(IEnumerable<TSource> sources)
