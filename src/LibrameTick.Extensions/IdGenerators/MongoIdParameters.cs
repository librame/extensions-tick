...
/// 定义 MonggoDB 标识参数。
/// </summary>
public struct MongoIdParameters : IEquatable<MongoIdParameters>
{
    private byte[] _timestampBytes;


    /// <summary>
    /// 使用字节数组构造一个 <see cref="MongoIdParameters"/>。
    /// </summary>
    /// <param name="timestampBytes">给定的时间戳字节数组。</param>
    public MongoIdParameters(byte[] timestampBytes)
    {
    ...
    /// 比较是否相等。
    /// </summary>
    /// <param name="other">给定的 <see cref="MongoIdParameters"/>。</param>
    /// <returns>返回布尔值。</returns>
    public bool Equals(MongoIdParameters other)
    ...
    /// <returns>返回 <see cref="MongoIdParameters"/>。</returns>
    public static MongoIdParameters Parse(string id)
        => new MongoIdParameters(id.FromHexString());
