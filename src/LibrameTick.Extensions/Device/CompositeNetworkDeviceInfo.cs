...
public readonly struct CompositeNetworkDeviceInfo : INetworkDeviceInfo, IComposable<INetworkDeviceInfo>
{
    ...
    public CompositeNetworkDeviceInfo(IEnumerable<INetworkDeviceInfo> infos)
    {
        ...
        _networkTypes = _infos.Select(s => s.NetworkType).Distinct().ToArray();
        _operationalStatuses = _infos.Select(s => s.Status).Distinct().ToArray();

        Traffic = new(infos.Select(s => s.Traffic.CreateTime).Max(),
            infos.Select(s => s.Traffic.ReceivedLength).Sum(),
            infos.Select(s => s.Traffic.SendLength).Sum(),
            infos.Select(s => s.Traffic.ReceivedRate).Average(),
            infos.Select(s => s.Traffic.SendRate).Average());
    ...
    public string Id => string.Join(',', _infos.Select(s => s.Id));
    ...
    public string Mac => string.Join(',', _infos.Select(s => s.Mac));
    ...
    public string Description => string.Join(',', _infos.Select(s => s.Description));
    ...
    public long Speed => (long)_infos.Select(s => s.Speed).Average();

    /// <summary>
    /// 是否支持 IPv4。
    /// </summary>
    public bool IsSupportIPv4 => _infos.Any(p => p.IsSupportIPv4);

    /// <summary>
    /// 是否支持 IPv6。
    /// </summary>
    public bool IsSupportIPv6 => _infos.Any(p => p.IsSupportIPv6);

    /// <summary>
    /// 是否能够与其他计算机通讯（公网或内网），如果任何网络接口标记为 "up" 且不是环回或隧道接口，则认为网络连接可用。
    /// </summary>
    public bool IsAvailable => _infos.Any(p => p.IsAvailable);
    ...
    public IReadOnlyCollection<IPAddress> MulticastAddresses
        => _infos.SelectMany(s => s.MulticastAddresses).Distinct().AsReadOnlyCollection();
    ...
    public IReadOnlyCollection<IPAddress> UnicastAddresses
        => _infos.SelectMany(s => s.UnicastAddresses).Distinct().AsReadOnlyCollection();
    ...
    public IReadOnlyCollection<IPAddress> GatewayAddresses
        => _infos.SelectMany(s => s.GatewayAddresses).Distinct().AsReadOnlyCollection();
    ...
    public IReadOnlyCollection<IPAddress> DnsAddresses
        => _infos.SelectMany(s => s.DnsAddresses).Distinct().AsReadOnlyCollection();
