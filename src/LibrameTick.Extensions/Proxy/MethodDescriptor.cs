...
public class MethodDescriptor : IEquatable<MethodDescriptor>
{
    ...
    public MethodDescriptor(MethodInfo target, MethodInfo source, object?[]? args)
    {
        Target = target;
        Source = source;
        Args = args;
        ParamTypes = target.GetParameters().Select(s => s.ParameterType).ToArray();
    }
    ...
    /// 拦截的目标方法信息，通常是接口方法信息。
    /// </summary>
    public MethodInfo Target { get; init; }

    /// <summary>
    /// 拦截的来源方法信息，通常是接口实现的方法信息。
    /// </summary>
    public MethodInfo Source { get; init; }
    ...
    /// 方法参数类型数组。
    /// </summary>
    public Type[] ParamTypes { get; init; }
