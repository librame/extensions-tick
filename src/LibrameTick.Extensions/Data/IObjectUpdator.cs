...
public interface IObjectUpdator
{
    ...
    {
        var currentUpdatedBy = await GetObjectUpdatedByAsync(cancellationToken).DiscontinueCapturedContext();

        return await SetObjectUpdatedByAsync(newUpdatedByFactory(currentUpdatedBy), cancellationToken).DiscontinueCapturedContext();
    }
