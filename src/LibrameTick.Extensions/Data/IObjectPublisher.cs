...
public interface IObjectPublisher
{
    ...
    /// 设置对象发表者。
    /// </summary>
    /// <param name="newPublishedByFactory">给定的新发表者对象工厂方法。</param>
    /// <returns>返回发表者（兼容标识或字符串）。</returns>
    object? SetObjectPublishedBy(Func<object?, object?> newPublishedByFactory)
    {
        var currentPublishedBy = GetObjectPublishedBy();

        return SetObjectPublishedBy(newPublishedByFactory(currentPublishedBy));
    }
    ...
    {
        var currentPublishedBy = await GetObjectPublishedByAsync(cancellationToken).DiscontinueCapturedContext();

        return await SetObjectPublishedByAsync(newPublishedByFactory(currentPublishedBy), cancellationToken).DiscontinueCapturedContext();
    }
