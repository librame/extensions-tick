...
/// <summary>
/// 定义支持 <see cref="DateTimeOffset"/> 的 CRON 表达式。
/// </summary>
public class CronExpressionOffset : AbstractCronExpression<DateTimeOffset>
{
    /// <summary>
    /// 构造一个 <see cref="CronExpressionOffset"/>。
    /// </summary>
    /// <param name="expression">给定的 CRON 表达式字符串。</param>
    /// <param name="options">给定的 CRON 选项。</param>
    public CronExpressionOffset(string expression, CronOptions options)
    ...
    protected override int GetMaxYear(DateTimeOffset? baseTimeOffset)
        => (baseTimeOffset ?? DateTimeOffset.UtcNow).Year + 100;
    ...
    public override DateTimeOffset? GetNextOccurrenceTime(DateTimeOffset baseTimeOffset)
    {
        // 向前移动一秒钟，因为我们正在计算时间*之后*
        baseTimeOffset = baseTimeOffset.AddSeconds(1);

        // 不处理毫秒
        var date = baseTimeOffset.WithoutMillisecond();
        ...
        while (!gotOne)
        {
            ...
            if (dayOfMSpec && !dayOfWSpec)
            {
                ...
                if (Descriptor.LastdayOfMonth)
                {
                    ...
                    else
                    {
                        ...
                        var tcal = new DateTimeOffset(date.Year, mon, day, 0, 0, 0, date.Offset);
                        ...
                        var nTime = new DateTimeOffset(tcal.Year, mon, day, hr, min, sec, date.Millisecond, date.Offset);
                        if (nTime < baseTimeOffset) // nTime.ToUniversalTime()
                        {
                            day = 1;
                            mon++;
                        }
                ...
                else if (Descriptor.NearestWeekday)
                {
                    ...
                    tcal = new(tcal.Year, mon, day, hr, min, sec, date.Offset);
                    if (tcal < baseTimeOffset) // tcal.ToUniversalTime()
                    {
                        day = Descriptor.DaysOfMonth.First();
                        mon++;
                    }
                ...
                if (day != t || mon != tmon)
                {
                    if (mon > 12)
                    {
                        date = new DateTimeOffset(date.Year, 12, day, 0, 0, 0, date.Offset).AddMonths(mon - 12);
                    }
                    else
                    {
                        //这是为了避免从一个月移动时出现错误
                        ...
                        if (day <= lDay)
                        {
                            date = new(date.Year, mon, day, 0, 0, 0, date.Offset);
                        }
                        else
                        {
                            date = new DateTimeOffset(date.Year, mon, lDay, 0, 0, 0, date.Offset).AddDays(day - lDay);
                        }
                    }
                    continue;
                }
            ...
            else if (dayOfWSpec && !dayOfMSpec)
            {
                ...
                if (Descriptor.LastdayOfWeek)
                {
                    ...
                    {
                        if (mon == 12)
                            date = new DateTimeOffset(date.Year, mon - 11, 1, 0, 0, 0, date.Offset).AddYears(1);
                        else
                            date = new(date.Year, mon + 1, 1, 0, 0, 0, date.Offset);
                ...
                else
                {
                    ...
                    if (daysToAdd > 0)
                    {
                        date = new(date.Year, mon, day + daysToAdd, 0, 0, 0, date.Offset);
                        continue;
                    }
                }
            }
            ...
            if (mon != t)
            {
                date = new(year, mon, 1, 0, 0, 0, date.Offset);
                continue;
            }
            ...
            if (year != t)
            {
                date = new(year, 1, 1, 0, 0, 0, date.Offset);
                continue;
            }
