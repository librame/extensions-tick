...
public static class ExpressionMapper<TInput, TOutput>
{
    ...
    private static Func<TInput, TOutput> InitialMapAllPublicPropertiesFunc()
    {
        ...
        {
            if (!outProp.CanWrite || outProp.PropertyType.IsSameOrNullableArgumentType(outputType))
                continue; // 排除自引用类型

            var inProp = inputType.GetProperty(outProp.Name);
            if (inProp is not null && outProp.PropertyType.IsSameOrNullableArgumentType(inProp.PropertyType))
            {
    ...
    private static Func<TInput, TOutput> InitialMapAllFieldsFunc()
    {
        ...
        {
            if (outField.FieldType.IsSameOrNullableArgumentType(outputType))
                continue; // 排除自引用类型

            var inField = inputType.GetField(outField.Name, TypeExtensions.AllMemberFlags);
            if (inField is not null && outField.FieldType.IsSameOrNullableArgumentType(inField.FieldType))
            {
