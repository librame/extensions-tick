...
public sealed class PropertyNotifier : IPropertyNotifier
{
        ...
        bool isUpdate, Action action)
    {
        ...
        PropertyChanging?.Invoke(this,
            new PropertyNotifierChangingEventArgs(propertyName, changingValue, oldValue, isUpdate));

        action();

        // 调用属性改变后事件处理程序
        PropertyChanged?.Invoke(this,
            new PropertyNotifierChangedEventArgs(propertyName, changingValue, oldValue, isUpdate));
    }
        ...
        Action<object>? addedOrUpdatedAction = null)
    {
        ...
        {
            if (_propertyFuncs.TryGetValue(key, out var value))
            {
                var oldValueFunc = value;
        ...
        Action<TValue>? addedAction = null, Action<TValue>? gotAction = null)
    {
        ...
        if (_propertyValues.TryGetValue(key, out var propValue))
        {
            var currentValue = (TValue)propValue;
            gotAction?.Invoke(currentValue);

            return currentValue;
        }

        if (_propertyFuncs.TryGetValue(key, out var propFunc))
        {
            var currentValue = (TValue)propFunc();
            gotAction?.Invoke(currentValue);
