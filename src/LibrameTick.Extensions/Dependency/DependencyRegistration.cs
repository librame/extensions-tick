...
using InternalExtensionsDependencyContext = Librame.Extensions.Dependency.Internal.DependencyContext;
...
public static class DependencyRegistration
{
    ...
    /// <param name="initialDependencyFunc">给定依赖的初始方法。</param>
    ...
    public static TDependency InitializeDependency<TDependency>(
        Func<IDependencyContext, DependencyCharacteristic, TDependency> initialDependencyFunc,
        IDependencyContext? context = null)
        where TDependency : IDependency
    {
        var initializer = GetInitializer(initialDependencyFunc, out var characteristic);
    ...
    public static TDependency InitializeDependency<TInitializer, TDependency>(IDependencyContext? context = null)
        ...
        where TDependency : IDependency
        => InitializeDependency(() => new TInitializer(), context);

    /// <summary>
    /// 使用指定的初始化器方法初始化依赖。
    /// </summary>
    /// <typeparam name="TDependency">指定的依赖类型。</typeparam>
    /// <param name="initialFunc">给定的依赖初始化器方法（可空；如果调用前未注册则必选，否则将抛出 NULL 异常）。</param>
    /// <param name="context">给定的 <see cref="IDependencyContext"/>（可空；默认使用 <see cref="CurrentContext"/>）。</param>
    /// <returns>返回 <typeparamref name="TDependency"/>。</returns>
    public static TDependency InitializeDependency<TDependency>(Func<IDependencyInitializer<TDependency>>? initialFunc,
        IDependencyContext? context = null)
        where TDependency : IDependency
    {
        var initializer = GetInitializer(initialFunc, out var characteristic);
    ...
    public static IDependencyInitializer<TDependency> GetInitializer<TDependency>(
        ...
        where TDependency : IDependency
        => GetInitializer(() => new Internal.DependencyInitializer<TDependency>(initialDependencyFunc), out characteristic);
