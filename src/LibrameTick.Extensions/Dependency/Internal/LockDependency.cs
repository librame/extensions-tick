...
internal sealed class LockDependency(int maxLockersCount, TimeSpan deadlockDuration) : ILockDependency
{
    private bool _disposed = false;


    public LockDependency()
        : this(maxLockersCount: Environment.ProcessorCount, deadlockDuration: TimeSpan.FromSeconds(5))
    {
    }
    ...
    public void Mutex(Action action, bool initiallyOwned, string? name, out bool createdNew)
    {
        var mutex = new Mutex(initiallyOwned, name, out createdNew);
    ...
    public TResult Mutex<TResult>(Func<TResult> func, bool initiallyOwned, string? name, out bool createdNew)
    {
        var mutex = new Mutex(initiallyOwned, name, out createdNew);
