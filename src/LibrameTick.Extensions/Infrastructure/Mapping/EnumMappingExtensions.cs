...
public static class EnumMappingExtensions
{
    ...
    public static Dictionary<TEnum, TValue> GetEnumItems<TEnum, TValue>(Func<EnumMappingDescriptor<TEnum>, TValue> valueConverter)
        where TEnum : Enum
        => GetEnumItems(static descr => descr.EnumItem, valueConverter);
    ...
    public static Dictionary<TKey, TValue> GetEnumItems<TEnum, TKey, TValue>(
        Func<EnumMappingDescriptor<TEnum>, TKey> keyConverter,
        Func<EnumMappingDescriptor<TEnum>, TValue> valueConverter,
        IEqualityComparer<TKey>? comparer = null)
        where TEnum : Enum
        where TKey : notnull
        => EnumMapper<TEnum>.Map()
    ...
    public static Dictionary<TEnum, IEnumerable<TValue>> GetEnumItemsWithAttributes<TEnum, TAttribute, TValue>(
        Func<EnumMappingDescriptor<TEnum>, TAttribute, TValue> valueConverter)
        where TEnum : Enum
        where TAttribute : Attribute
        => GetEnumItemsWithAttributes(static key => key.EnumItem, valueConverter);
