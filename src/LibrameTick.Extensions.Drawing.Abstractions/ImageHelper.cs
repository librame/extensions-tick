...
public static class ImageHelper
{
    ...
    /// <param name="initialCoord">给定的初始化坐标。</param>
    /// <param name="isRandomCoord">是否生成随机坐标（可选；默认不随机）。</param>
    /// <returns>返回 <see cref="Point"/>。</returns>
    public static Point CalculateCoord(Size imageSize, Point initialCoord,
        bool isRandomCoord = false)
    {
        var startX = initialCoord.X;
        var startY = initialCoord.Y;
