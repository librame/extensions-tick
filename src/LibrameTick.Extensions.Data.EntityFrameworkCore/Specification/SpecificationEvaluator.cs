...
/// <summary>
/// 定义用于执行 <see cref="ISpecification{T}"/> 求值程序。
/// </summary>
public static class SpecificationEvaluator
{
    ...
    public static async Task<IList<T>> EvaluateListAsync<T>(IQueryable<T> queryable,
        ISpecification<T>? specification = null, CancellationToken cancellationToken = default)
        where T : class
    {
        if (specification is null)
    ...
    public static Task<IPagingList<T>> EvaluatePagingListAsync<T>(IQueryable<T> queryable,
        Action<IPagingList<T>> pageAction, ISpecification<T>? specification = null,
